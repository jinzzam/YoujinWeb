...
delete from member;
delete from member where id='kukaro1';
...
insert into member 
values ('김태정', date('1993-01-05'), 'm', 'kukaro1', 'kukuku', '박유진=왑최고귀요미');

update member set password = 'kakaka', gender = 'f' where id = 'kukaro1';
...
#권한 정하는법
use mysql; # mysql db로 들어가서 user테이블을 확인한다.
select * from user;
GRANT ALL on *.* TO 'root'@'localhost'; #안될때 해결책
flush privileges; #안될때 해결책